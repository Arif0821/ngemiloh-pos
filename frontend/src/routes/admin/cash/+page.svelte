<script lang="ts">
  ...
  function exportCSV() {
    ...
    const header = 'Shift Date,Kasir,Mulai Shift,Tutup Shift,Kas Awal,Penjualan Tunai Sistem,Kas Akhir Fisik,Selisih (Discrepancy),Status\n';
    ...
    <div class="overflow-x-auto">
      <table class="w-full text-left text-sm text-slate-600">
        ...
        <tbody class="divide-y divide-slate-200">
          {#if isLoading}
            <tr><td colspan="8" class="text-center py-10 text-slate-500">Loading...</td></tr>
          {:else if shifts.length === 0}
            <tr><td colspan="8" class="text-center py-10 text-slate-500">Belum ada riwayat shift.</td></tr>
          {:else}
            {#each shifts as shift}
              <tr class="hover:bg-slate-50">
                ...
                <td class="px-4 py-3 text-right">{shift.system_cash_total ? formatRp(Number(shift.system_cash_total) - Number(shift.opening_balance)) : '-'}</td>
                <td class="px-4 py-3 text-right font-bold text-slate-800">{formatRp(Number(shift.closing_balance))}</td>
                <td class="px-4 py-3 text-right font-bold {Number(shift.discrepancy) < 0 ? 'text-red-600' : 'text-slate-600'}">
                  {formatRp(Number(shift.discrepancy))}
                </td>
                <td class="px-4 py-3 text-center">
                  {#if shift.status === 'open'}
                    <span class="px-2 py-1 bg-blue-100 text-blue-700 rounded-full text-xs font-bold uppercase">Open</span>
                  {:else}
                    <span class="px-2 py-1 bg-slate-100 text-slate-700 rounded-full text-xs font-bold uppercase">Closed</span>
                  {/if}
                </td>
              </tr>
            {/each}
