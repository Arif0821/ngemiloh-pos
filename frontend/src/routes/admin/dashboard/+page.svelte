			...
			if (res.ok) {
				...
				if (paymentChart) {
					paymentChart.data.datasets[0].data = [
						kpi.paymentDistribution.cash,
						kpi.paymentDistribution.qris,
						kpi.paymentDistribution.split
					];
					paymentChart.update();
				}
			}
	...
	<div class="grid grid-cols-1 gap-6 md:grid-cols-3">
		...
		>
			...
			<h3 class="text-surface-900 dark:text-surface-50 text-3xl font-black">
				{formatRp(kpi.revenue)}
			</h3>

			<div class="mt-4">
				<div class="mb-1 flex justify-between text-xs">
					<span class="text-surface-500 font-bold">Progress Target (5Jt)</span>
					<span class="text-brand-600 font-bold">{kpi.targetProgress}%</span>
				</div>
