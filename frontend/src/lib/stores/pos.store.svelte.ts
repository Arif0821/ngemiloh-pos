...
export class PosStore {
	...
	splitQrisAmount: number = $derived(
		Math.max(0, this.cartTotal - Number(this.splitCashAmount || 0))
	);
	cashChange: number = $derived(
		this.paymentMethod === 'cash' ? Number(this.cashAmount || 0) - this.cartTotal : 0
	);
		...
		for (const d of this.activeDiscounts) {
			if (!d.applicable_days || d.applicable_days.includes(today)) {
				if (
					...
					const amt =
						d.type === 'percentage'
							? Number(product.base_price) * (Number(d.value) / 100)
							: Number(d.value);
					if (amt > maxAmount) {
						maxAmount = amt;
