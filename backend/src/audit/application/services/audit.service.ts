...
export class AuditService implements OnModuleInit {
  ...
  async getLogs(
    ...
    const [logs, total] = await this.auditRepository.findLogs(
      parsedFilters,
      skip,
      limit,
    );
  ...
  async archiveOldLogs() {
    ...
    const deletedCount =
      await this.auditRepository.deleteLogsOlderThan(threeYearsAgo);

    this.logger.log(
      `Archived ${deletedCount} audit logs older than 3 years to ${filename}`,
    );
