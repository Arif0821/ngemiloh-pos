...
export class PrismaDiscountRepository implements IDiscountRepository {
  ...
  async create(data: Prisma.DiscountUncheckedCreateInput, adminId: string): Promise<Discount> {
    return this.prisma.$transaction(async (tx) => {
      ...
      await tx.auditLog.create({
        data: {
          ...
          new_value: { code: created.name, value: Number(created.value), type: created.type } as Prisma.InputJsonValue
        }
      });
  ...
  async update(id: string, data: Prisma.DiscountUncheckedUpdateInput): Promise<Discount> {
